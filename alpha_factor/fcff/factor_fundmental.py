...
#        self.prevDate = self.getPrevTradingDays(self.today)
    ...
    def wind_trade_day(self):
        sql="select TRADE_DAYS from WIND.ASHARECALENDAR where (S_INFO_EXCHMARKET='SZSE') and (TRADE_DAYS>=%s) and (TRADE_DAYS<=%s)"%(self.initial, self.today)
...
#    def cal_season
    ...
    #        today = '20110412'
                ...
                ttm_df.to_csv('%s/%s_%s_%s.csv'%(self.factor_path,self.factor_filename, self.cal_mode, today1),index=None,header=None)
#                return ttm_df
...
#            return ttm_df
            ...
            ttm_df.to_csv('%s/%s_%s_%s.csv'%(self.factor_path,self.factor_filename, self.cal_mode, self.today),index=None,header=None)
            return ttm_df
...
#        i = '000806.SZ'
        ...
        for i in range(len(code)):
            ...
            if isinstance(indicator_ttm,float) and np.isnan(indicator_ttm):  ##数据里的nan 都是float64格式的 而np.nan == np.nan 这种方式是无效的
