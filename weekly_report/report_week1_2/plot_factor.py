...
def getRetPlot(path, start, end):
    ...
    ret_1week = df_1week.copy()
    df_1week = (df_1week + 1).cumprod()
    df_1week = df_1week.round(3)
    
    col = df.columns.tolist()
    col1 = col[:10]
    col2 = col[10:]
    return df_1week, df_2week, df_month, df_q, df_year, col1, col2
...
def run(path, start, end):
    df_1week, df_2week, df_month, df_q, df_year, col1, col2 = getRetPlot(path, start, end)
    
    getPng(df_1week, col1, u"Barra风险因子 最近一周", 'factor_month', 35)
    getPng(df_2week, col1, u"Barra风险因子 最近两周", 'factor_year', 35)
    
    getPng(df_1week, col2, u"申万行业因子 最近一周", 'industry_month', 45)
    getPng(df_2week, col2, u"申万行业因子 最近两周", 'industry_year',45)
