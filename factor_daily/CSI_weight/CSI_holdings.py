...
def get_holding(day ,code, dirpath, output_path, get_db):
    table = DBF('%s/GZ%s%s.dbf'%(dirpath,code,day), load=True, encoding='iso-8859-1')
    ...
    df1 = df[df['A0'].isin(['1002','12020101204001', '3003'])]
...
def run(start, end, today, code, dirpath, output_path,flag, fre):
    ...
    if flag==0:
        for day in tradeday:
           print(day)
           get_holding(day, code, dirpath, output_path, getdb)
    else:
        get_holding(today, code,dirpath, output_path, getdb)
...
if __name__ == '__main__':
    ...
    output_path = 'Z:/daily_data/holding/production/account-ZZ500-pdt'
    fre = 'day'
    flag = 1
    code ='722323'
    run(start, end, today, code, dirpath, output_path,flag, fre)